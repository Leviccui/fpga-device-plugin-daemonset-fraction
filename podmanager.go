...
func kubeInit() {
	...
	//kubeconfigFile := "/home/dell/.kube/config"
	var err error
	var config *rest.Config

	if _, err = os.Stat(kubeconfigFile); err != nil {
		log.Infof("kubeconfig %s failed to find due to %v", kubeconfigFile, err)
...
func getPendingPodsInNode() ([]v1.Pod, error) {
	...
	log.Debugf("all pod list %v", podList.Items)
...
// pick up the FPGA pod with assigned status is false, and
func getCandidatePods() ([]*v1.Pod, error) {
	log.Infof("entry getCandidatedPods()")
	candidatePods := []*v1.Pod{}

	log.Infof("entry getPendingPodsInNode()")
	allPods, err := getPendingPodsInNode()
	log.Infof("exit getPendingPodsInNode()")
	if err != nil {
		return candidatePods, err
	}

	log.Infof("start checking FPGAAssumed")
	for _, pod := range allPods {
		current := pod
		if isFPGAAssumedPod(&current) {
			candidatePods = append(candidatePods, &current)
		}
	}
	log.Infof("finish checking FPGAAssumed")

	for _, pod := range candidatePods {
		log.Debugf("candidate pod %s in ns %s with timestamp %d is found.",
			pod.Name,
			pod.Namespace,
			getAssumeTimeFromPodAnnotation(pod))
	}

	log.Infof("entry makePodOrderdByAge(candidatePods)")
	podOrderdByAge := makePodOrderdByAge(candidatePods)
	log.Infof("exit makePodOrderdByAge(candidatePods)")

	return podOrderdByAge, nil
}

// make the pod ordered by FPGA assumed time
